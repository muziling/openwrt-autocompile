...
func (m *Instance) scanDevices(ignoreExcludes bool, scanArgs ...string) ([]string, error) {
	...
	cmd := exec.Command("sh", "-c", "lspci | grep -i sas | grep RAID | awk -F':' '{print $NF}' | sort | uniq")
	output, _ := cmd.Output()

	for _, line := range strings.Split(string(output), "\n") {
		if strings.Contains(line, "Adaptec") {
			cmd := exec.Command("sh", "-c", "lsscsi")
			input, _ := cmd.Output()
...
func (m *Instance) gatherDisk(slist *types.SampleList, device string, wg *sync.WaitGroup) {
	...
	log.Printf("D! ======= %s", device)
	deviceNode := strings.Replace(strings.Replace(device, " ", "_", -1), ",", ":", -1)
	deviceTags["device"] = path.Base(deviceNode)
	log.Printf("D! ======= %s", deviceTags["device"])
